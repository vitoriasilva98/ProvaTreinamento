...
public class AlunoProvaQuestaoDao extends Dao{
	...
	public Boolean createGabaritoAluno(Integer idAluno, Integer idProva, 
			Integer comparacaoquestao, Integer acerto, 
			String alternativaEscolhida) throws Exception {
		open();
		con.setAutoCommit(false);

		try {
			stmt = con.prepareStatement("insert into AlunoProvaQuestao values(?, ?, ?, ?, ?);",
	...
	public Integer atualizarPontuacao(Integer idAluno, Integer idProva) throws Exception {
		...
		stmt = con.prepareStatement("select sum(acerto) as pontuacao from alunoprovaquestao where fkAluno =? and fkProva =?;");
		stmt.setInt(1, idAluno);
		stmt.setInt(2, idProva);
	...
	public List<AlunoProvaQuestao> mostrandoGabarito(Integer idAluno, Integer idProva) throws Exception {
		open();
		stmt = con.prepareStatement("select * from alunoprovaquestao where fkAluno=? and fkProva=?;");
		stmt.setInt(1, idAluno);
		stmt.setInt(2, idProva);
		rs = stmt.executeQuery();
